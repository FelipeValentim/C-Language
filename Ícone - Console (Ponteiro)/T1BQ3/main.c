    ...
    do{
        do{
            ...
            if((qtdIcones < 1 ) && (opt != 1)  && (opt > 1) && (opt < 8)){ //Apenas visual
                printf("Sem icones registrados. Adicione pelo menos um!\n\n"); //Printa a mensagem caso selecione uma das opções que envolve a existência de pelo menos um icone
            }
        }while((qtdIcones < 1) && (opt != 1) && (opt != 8)); //Loop que repete até que o usuário adicione pelo menos 1 icone
        switch(opt)
        {
        case 1:
        ...
        case 6:
            ...
            break;
        case 7:
            ico = checa_simetria(ico);
            break;
        case 8:
            ico = libera_memoria(ico);
            exit(0);
        }
    } while(opt != 8);
    return 0;
